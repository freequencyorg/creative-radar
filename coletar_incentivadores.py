...
UF, tipo de pessoa, responsável e total doado acumulado. O histórico doação a
...
Só biblioteca padrão do Python 3. Uso:
  ...
  python3 coletar_incentivadores.py                     # pessoas jurídicas
  python3 coletar_incentivadores.py --pessoas todas     # inclui pessoas físicas
...
def coletar(base, pessoas, minimo, limite, pausa, log):
    ...
    guardados, erros = {}, []
    lidos, offset, paginas, total_informado = 0, 0, 0, 0
    while True:
        ...
        for reg in itens:
            ...
            if r["total_doado"] < minimo:
                continue
            chave = r.get("cgccpf") or r.get("id") or r["nome"]
...
def main():
    ...
    ap.add_argument("--pessoas", default="juridica", choices=["juridica", "fisica", "todas"])
